...
[3] Containment과 Specialization을 같이 사용하기
...
* 예제이서처럼 props를 통해 데이터를 전달하는 기존 방식은 실제 데이터를 필요로 하는 컴포넌트까지의 깊이기 깊어질수록 복잡해집니다.
...
* 하지만 무조건 컨텍스트를 사용하는 것이 좋은것은 아닙니다
* 왜냐하면 컴포넌트의 컨텍스트가 연동되면 재사용성이 떨어지기 때문입니다
...
* 하위 컴포넌트는 가장 가까운 상위 레벨의 Provider로부터 컨텍스트를 받게 되지만, 만일 Provider을 찾을 수 없다면 설정한 기본값을 사용하게 됩니다
...
* 컨택스트 객체는 displayName이라는 문자열 속성을 갖습니다
* 크롬의 리액트 개발자 도구에서는 컨텍스트의 Provider나 Consumer을 표시할 때 displayName을 함꼐 표시해줍니다

14.5 여러 개의 컨텍스트 사용하기

* 여러 개의 컨텍스트를 동시에 사용하려면 Context.Provider을 중첩해서 사용합니다

14.6 useContext

* 함수형 컴포넌트에서 컨텍스트를 사용하기 위해 컴포넌트를 매번 Consuner 컴포넌트로 감싸주는 것보다 더 좋은 방법이 있습니다. 바로 7장에서 배운 Hook입니다
...
* Cascading이란 계단식이라는 뜻으로 한 엘리먼트에 여러 스타일이 적용될 경우 스타일같의 충동을 막기 위해 계단식으로 스타일을 적용시키는 규칙을 갖고 있습니다
...
* CSS문법을 그대로 사용하면서 결과물을 스타일링된 컴포넌트 형대로 만들어주는 오픈소스 라이브러리입니다
